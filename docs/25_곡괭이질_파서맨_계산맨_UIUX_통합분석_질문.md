# 곡괭이질·곡괭이전나라시·파서맨·계산맨·UIUX 통합 분석
...
| 메타 prefix 분류 | 원본 컬럼과 메타 컬럼을 분리해야 함. 단, 파서맨 후보 컬럼을 별도로 늘리지 않는다 | `_api_*`, `_calc_*`, `_validation_*` |
...
| 버릴 결합 | 새 구조 | 예시 |
|---|---|---|
| Raw 화면 함수가 전체 파이프라인을 직접 제어 | 오케스트레이터가 단계 호출 | `전처리실행기`가 파서맨/계산맨/적격매칭 호출 |
| mapAiToNotice에 특수 분기 집중 | 컬럼룰과 후처리 모듈로 분산 | 종목, 지역, 보험료를 각각 후처리기로 분리 |
| 파서맨 결과와 계산 결과가 같은 patch에 섞임 | 출처별 후보와 최종값 분리 | parser candidate, calculation result, final value |
...
## Q-09. `종목/발주처/대분류`는 파서맨 후보 필터에 쓰지 않는 것으로 확인하면 될까?
...
## Q-11. 파서맨이 저장할 근거는 어느 수준까지 필요할까?
...
## Q-22. 파서맨 화면은 기존 iframe 구조를 버리고 앱 내부 화면으로 새로 만들까?
...
새 구조:
룰관리 > 파서맨 화면 안에서 룰 목록, 테스트, 가이드를 같은 UI로 제공.
```
...
파서맨 후보값은 별도 `_parser_*` 컬럼을 만들지 않고 대상 컬럼에 덮어쓴다.
```
